...
import json # For storing JSON data in TestResult answers
...
def start_test(request, test_id):
    ...
    if test.is_placement_test:
        ...
        if current_placement_test_result:
            ...
            test_result = TestResult.objects.create(
                ...
                # لا نحدد score أو determined_level_at_this_stage هنا، لأنه سيتم حسابهما عند الإرسال
                # answers = json.dumps({'current_stage': test.level, 'answered_questions': {}})
                # Note: 'answers' field now primarily stores the selected options, not stage info.
            )
        ...
        context = {
            ...
            'questions': questions,
            'is_placement_test': True, # لتسهيل التمييز في القالب
        }
        return render(request, 'academic/start_test.html', context) # استخدم قالب start_test.html الموحد
    ...
    # منطق الاختبارات العادية (ليست تحديد مستوى)
        ...
        test_result = TestResult.objects.create(
            test=test,
            student=student_user,
            status='in_progress', # حالة جديدة لـ "قيد التقدم" للاختبارات العادية
            score=0,
        )
...
def submit_test(request, test_result_id):
    ...
    if form.is_valid():
        # حفظ إجابات الطالب
        student_answers_dict = form.student_answers # هذه هي إجابات الطالب من دالة clean في الفورم

        # تحديث حقل 'answers' في TestResult. 
        # هذا الحقل سيخزن الإجابات لكل سؤال (question_id: selected_option_id)
        # لاحظ أننا لم نعد نخزن 'current_stage' هنا، بل نعتمد على 'test.level' و 'test.next_test_on_success'
        test_result.answers = json.dumps(student_answers_dict)
        
        # حفظ النتيجة
        test_result.score = form.score # النتيجة المحسوبة من دالة clean في الفورم
        test_result.end_time = timezone.now()
        
        max_score = sum(q.score_points for q in questions_to_evaluate)
        ...
        # ----------------------------------------------------------------------
        if test.is_placement_test:
            # عتبات التوجيه (يمكن تعديلها في مكان واحد هنا أو في settings)
            # بما أننا نستخدم levels مثل A1, A2, B1، فسنعتمد على هذه المستويات
            # ونفترض أن الانتقال يعني نجاحًا في المستوى الحالي للانتقال للأعلى
            # أو فشلاً للرجوع للأدنى.
            # النسبة المئوية للنجاح في مرحلة معينة للانتقال للأعلى
            SUCCESS_THRESHOLD = 80 # للانتقال للمستوى التالي (higher level)
            # النسبة المئوية للنجاح لتحديد المستوى في هذه المرحلة
            PASS_THRESHOLD = 50 # لتحديد مستوى الطالب بهذا المستوى

            test_result.status = 'completed' # هذه المرحلة من الاختبار اكتملت
            ...
            if percentage_score >= SUCCESS_THRESHOLD and test.next_test_on_success:
                ...
                if test.next_test_on_failure:
                    ...
                    student_user.determined_arabic_level = 'A1' # أو يمكن أن تظل 'unassigned' إذا أردت تدخل إداري
                    student_user.save()
                    test_result.is_final_placement_result = True
                    test_result.status = 'finalized'
                    test_result.save()
    ...
    try:
        ...
        if arabic_course:
            ...
            available_classes = Class.objects.filter(
                ...
                required_arabic_level__in=[determined_level, 'any'],
